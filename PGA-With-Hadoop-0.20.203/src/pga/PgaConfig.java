...
{
	public static boolean USEPREGELMERGE = false;
	public static int NOWORKERS=-1;
	// important paths
	public static String hadoopReadPath = null;
	public static String hadoopBasePath = null;
	public static String localBasePath = "work";
	public static String userName = System.getProperty("user.name"); //Argo: added
	public static String trackSSPath = System.getProperty("user.home"); //Argo: added
	public static String hadoopHomePath = System.getenv("HADOOP_HOME"); //Argo: added
	...
	public static String zookeeperPort = "2181"; //Argo Added to provide zookeeper server lis
	...
	public static void validateConfiguration()
	{
		...
		if(USEPREGELMERGE== true)
		{
			if(NOWORKERS == -1)
			{
				System.err.println("ERROR: For running on pregel you need to mention number of workers");
			}
		}
	...
	{
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		try { //Argo Added to provide zookeeper server list
			BufferedReader br = null;  //Argo Added to provide zookeeper server list
			String sCurrentLine; //Argo Added to provide zookeeper server list
 			br = new BufferedReader(new FileReader(trackSSPath + "/zklist")); //Argo Added to provide zookeeper server list
 			while ((sCurrentLine = br.readLine()) != null) { //Argo Added to provide zookeeper server list
				System.out.println(sCurrentLine); //Argo Added to provide zookeeper server list
				nodeList += sCurrentLine + ":" + zookeeperPort + ","; //Argo Added to provide zookeeper server list
			} //Argo Added to provide zookeeper server list
 			br.close(); //Argo Added to provide zookeeper server list
 		}  //Argo Added to provide zookeeper server list
		catch (IOException e) { e.printStackTrace(); } //Argo Added to provide zookeeper server list
		nodeList = nodeList.substring(0, (nodeList.length()-1)); //Argo Added to provide zookeeper server list
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		validateConfiguration();
		
		Pga.msg("Pga " + Pga.VERSION + "\n");
		Pga.msg("==================================================================================\n");
		Pga.msg("Input: "         + hadoopReadPath + "\n");
		Pga.msg("Workdir: "       + hadoopBasePath  + "\n");
		Pga.msg("localBasePath: " + localBasePath + "\n");
		
		Pga.msg("HADOOP_MAPPERS = "    + HADOOP_MAPPERS + "\n");
		Pga.msg("HADOOP_REDUCERS = "   + HADOOP_REDUCERS + "\n");
		Pga.msg("HADOOP_JAVA_OPTS = "  + HADOOP_JAVAOPTS + "\n");
		Pga.msg("HADOOP_TIMEOUT = "    + HADOOP_TIMEOUT + "\n");
		Pga.msg("HADOOP_LOCALNODES = " + HADOOP_LOCALNODES + "\n");
		
		if (STARTSTAGE != null)  { Pga.msg("STARTSTAGE = " + STARTSTAGE + "\n"); }
		if (STOPSTAGE  != null) { Pga.msg("STOPSTAGE = "  + STOPSTAGE + "\n");  }
		
		Pga.msg("RESTART_INITIAL = " + RESTART_INITIAL + "\n");
			
		Pga.msg("RESTART_TIP = " + RESTART_TIP + "\n");
		Pga.msg("RESTART_TIP_REMAIN = " + RESTART_TIP_REMAIN + "\n");
		
		Pga.msg("RESTART_COMPRESS = " + RESTART_COMPRESS + "\n");
		Pga.msg("RESTART_COMPRESS_REMAIN = " + RESTART_COMPRESS_REMAIN + "\n");
		
		Pga.msg("RESTART_SCAFF_PHASE = " + RESTART_SCAFF_PHASE + "\n");
		Pga.msg("RESTART_SCAFF_STAGE = " + RESTART_SCAFF_STAGE + "\n");
		Pga.msg("RESTART_SCAFF_FRONTIER = " + RESTART_SCAFF_FRONTIER + "\n");
		
		Pga.msg("PREPROCESS_SUFFIX = " + PREPROCESS_SUFFIX + "\n");

		Pga.msg("K = "               + K + "\n");
		Pga.msg("MAXR5 = "           + MAXR5 + "\n");
		Pga.msg("MAXTHREADREADS = "  + MAXTHREADREADS + "\n");
		Pga.msg("TRIM5 = "           + TRIM5 + "\n");
		Pga.msg("TRIM3 = "           + TRIM3 + "\n");
		Pga.msg("RECORD_ALL_THREADS = " + RECORD_ALL_THREADS + "\n");

		Pga.msg("TIPLENGTH = "       + TIPLENGTH + "\n");

		Pga.msg("MAXBUBBLELEN = "    + MAXBUBBLELEN + "\n");
		Pga.msg("BUBBLEEDITRATE = "  + BUBBLEEDITRATE + "\n");
		
		Pga.msg("LOW_COV_THRESH = "  + LOW_COV_THRESH + "\n");
		Pga.msg("MAX_LOW_COV_LEN = " + MAX_LOW_COV_LEN + "\n");
		
		Pga.msg("MIN_THREAD_WEIGHT = " + MIN_THREAD_WEIGHT + "\n");
		
		Pga.msg("INSERT_LEN = "     + INSERT_LEN + "\n");
		Pga.msg("MIN_WIGGLE = "     + MIN_WIGGLE + "\n");
		Pga.msg("MIN_CTG_LEN = "    + MIN_CTG_LEN + "\n");
		Pga.msg("MIN_UNIQUE_COV = " + MIN_UNIQUE_COV + "\n");
		Pga.msg("MAX_UNIQUE_COV = " + MAX_UNIQUE_COV + "\n");
		Pga.msg("MAX_FRONTIER = "   + MAX_FRONTIER + "\n");
		
		Pga.msg("RUN_STATS = " + RUN_STATS + "\n");
		Pga.msg("N50_TARGET = " + N50_TARGET + "\n");
		
		Pga.msg("CONVERT_FA = " + CONVERT_FA + "\n");
		Pga.msg("PRINT_FA = " + PRINT_FA + "\n");
		Pga.msg("FASTA_MIN_LEN = " + FASTA_MIN_LEN + "\n");
		Pga.msg("FASTA_MIN_COV = " + FASTA_MIN_COV + "\n");
		
		Pga.msg("Zk Server List:" + nodeList);
	...
	public static void parseOptions(String [] args)
	{
		...
		options.addOption(new Option("runPregelMerge",       "Run merge phase on pregel Giraph"));
		options.addOption(OptionBuilder.withArgName("w").hasArg().withDescription("if use pregel giraph then you need mention no workers").create("w"));
		// error correction
	    ...
	    try 
	    {
	        ...
	        {
	        	System.out.print("Pga version " + Pga.VERSION + " (Schatz et al. http://pga-bio.sf.net)\n" +
	        			         "\n" +
	        			         "Usage: Pga [-asm dir] [-reads dir] [-k k] [options]\n" +
	        			         "\n" +
	        			         "Pga Stages\n" +
	        	                 "================\n" +
	        	...
	        	System.out.print("\n" +
	    		                 ...
	    		                 "$ hadoop jar pga.jar -asm /path/to/newassembly -start scafolding <...>\n\n" +
	        ...
	        if (line.hasOption("runPregelMerge"))  { USEPREGELMERGE  = true; }
	        if(line.hasOption("w")){ NOWORKERS= Integer.parseInt(line.getOptionValue("w"));}
