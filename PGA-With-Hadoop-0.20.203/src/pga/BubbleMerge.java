...
public class BubbleMerge implements Tool {

	private static boolean local = false;
	private static final Logger sLogger = Logger.getLogger(BubbleMerge.class);
	public static void main(String[] argArray) throws Exception {
		...
		conf.setComputationClass(pga.BubbleComputation.class);
		...
		conf.setMasterComputeClass(pga.BubbleMaster.class);
		...
		GiraphJob job = new GiraphJob(conf, "BubbleMerge");
		cleanUnwantedFiles(job,argArray[0]);
		GiraphFileInputFormat.addVertexInputPath(conf, new Path(argArray[0]));
		BubbleMerge.removeAndSetOutput(job, new Path(argArray[1]));
	...
	public int run(String[] argArray) throws Exception {
		...
		conf.setBoolean("giraph.useSuperstepCounters", false);
		conf.setComputationClass(pga.BubbleComputation.class);  //compression in giraph
		conf.setVertexInputFormatClass(pga.GraphTextInputFormat.class);
		conf.setVertexOutputFormatClass(pga.GiraphTextOutputFormat.class);
		conf.setMasterComputeClass(pga.BubbleMaster.class);  //to halt supersteps
		if (local == true) {
			conf.setWorkerConfiguration(1, 1, 100.0f);
			// Single node testing
			GiraphConstants.SPLIT_MASTER_WORKER.set(conf, false);
