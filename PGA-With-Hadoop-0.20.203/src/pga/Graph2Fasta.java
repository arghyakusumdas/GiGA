...
{	
    ...
    implements Mapper<LongWritable, Text, Text, Text> 
	{
                ...
                throws IOException 
        {
			...
			reporter.incrCounter("Pga", "allnodes", 1);
			...
			{ 
				reporter.incrCounter("Pga", "printednodes", 1);
			
				output.collect(new Text(">" + node.getNodeId()), new Text("len=" + str.length() + "\tcov=" + node.cov())); //Argo: commented
	...
	/*public static class Graph2FastaReducer 
	...
	implements Reducer<Text, Text, Text, Text>{
		...
		throws IOException{
			...
			while (values.hasNext()) {
				//System.out.println("values.next().get() = " + values.next().get());
				sum += values.next();
			}
	...
	public void printFasta(String inputPath) throws Exception
	{
		...
		while (nodetxt != null)
		{
			...
			if ((len >= PgaConfig.FASTA_MIN_LEN) && (cov >= PgaConfig.FASTA_MIN_COV))
			{
	...
	{
		//String inputPath  = "/users/mschatz/pga/Ec100k/99-final/";
		//String outputPath = "/users/mschatz/pga/Ec100k/99-final.fa/";

		String inputPath  = "/users/mschatz/pga/Ec100k/04-notipscmp";
		String outputPath = "/users/mschatz/pga/Ec100k/04-notipscmp.fa";
		
		PgaConfig.FASTA_MIN_LEN = 100;
		PgaConfig.FASTA_MIN_COV = 2.0f;
