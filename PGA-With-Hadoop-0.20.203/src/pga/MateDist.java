...
{	
	...
	implements Mapper<LongWritable, Text, Text, Text> 
	{
		...
		throws IOException 
		{
			...
			if (isunique) { reporter.incrCounter("Pga", "unique_ctg", 1); }
			...
			if (isunique && reads != null)
			{
				...
				for(String readstr : reads)
				{
					...
					if (basename != null)
					{
						if (contiginfo.containsKey(basename))
						{
							...
							if (ok)
							{
								reporter.incrCounter("Pga", "internal_mates",  1);
								reporter.incrCounter("Pga", "internal_dist",   idist);
								reporter.incrCounter("Pga", "internal_distsq", (idist*idist));
							}
							else
							{
								reporter.incrCounter("Pga", "internal_invalidd", 1);
							}
						}
						else
						{
				...
				for (String basename : contiginfo.keySet())
				{
					...
					if (!info.internal)
					{
						...
						reporter.incrCounter("Pga", "linking_reads", 1);
					}
				}
			}
		}
		...
		throws IOException 
		{
			...
			if ((lnk1 != null) && (lnk2 != null))
			{
				...
				if (lnk1.unique || lnk2.unique)
				{
					...
					reporter.incrCounter("Pga", "linking_edges", 1);
