...
public class GraphMerge implements Tool {

	private static boolean local = false;
	private static final Logger sLogger = Logger.getLogger(GraphMerge.class);
	public static void main(String[] argArray) throws Exception {
		...
		conf.set("rand", ""+rand);
		conf.setComputationClass(pga.MergeComputation.class);
		...
		conf.setMasterComputeClass(pga.MasterVertex.class);
		...
		GiraphJob job = new GiraphJob(conf, "GraphMerge");
		...
		//System.exit(ToolRunner.run(new GraphMerge(), args));
	}
	...
	public int run(String[] argArray) throws Exception {
		...
		//conf.set("K", argArray[3]);
		conf.setComputationClass(pga.MergeComputation.class);  //compression in giraph
		conf.setVertexInputFormatClass(pga.GraphTextInputFormat.class);
		conf.setVertexOutputFormatClass(pga.GiraphTextOutputFormat.class);
		conf.setMasterComputeClass(pga.MasterVertex.class);  //to halt supersteps
		...
		//conf.setStrings(GiraphConstants.ZOOKEEPER_LIST, "mike058:2181,mike067:2181,mike087:2181");
		//Some of the settings for zookeeper and large graphs
		
		//GiraphConstants.ZOOKEEPER_JAVA_OPTS.set(conf, "-Xmx5g -XX:ParallelGCThreads=4 -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=70 -XX:MaxGCPauseMillis=100");
