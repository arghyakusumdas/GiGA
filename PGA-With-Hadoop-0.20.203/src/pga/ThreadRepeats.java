...
{	
	...
	implements Reducer<Text, Text, Text, Text> 
	{
		...
		throws IOException 
		{
			...
			else if (tandem)
			{
				...
				reporter.incrCounter("Pga", "tandem", 1);
			}
			else if ((fd == 0) || (rd == 0))
			{
				// Deadend node, could be a palidrome though
				reporter.incrCounter("Pga", "deadend", 1);
			...
			else if (false) // && (($fd == 1) || ($rd == 1)))
			{
				...
				if (!valid)
				{
					reporter.incrCounter("Pga", "invalidhalf", 1);
					System.err.println("Invalid Half for " + node.getNodeId());
				}

				reporter.incrCounter("Pga", "halfdecision", 1);
				reporter.incrCounter("Pga", "threadible", 1);
			...
			else
			{
				...
				if ((fd < 2) || (rd < 2)) { reporter.incrCounter("Pga", "halfdecision", 1); }
				...
				if (haveall)
				{
					reporter.incrCounter("Pga", "xcut", 1);
					reporter.incrCounter("Pga", "threadible", 1);
	...
	{ 
		...
		conf.setJobName("ThreadRepeats " + inputPath + " " + PgaConfig.K);
