...
{	
    ...
    implements Mapper<LongWritable, Text, Text, Text> 
	{
		...
		throws IOException 
		{
			...
			if ((len <= MAX_LOW_COV_LEN) && (cov < LOW_COV_THRESH))
			{
				...
				reporter.incrCounter("Pga", "lowcovremoved", 1);
				...
				if (degree == 0)
				{
					reporter.incrCounter("Pga", "lowcoverage_island", 1);
				}
			}
			else
			{
			...
			else if (sawnode == 1)
			{
				if (edges.size() > 0)
				{
					for(EdgeInfo edge : edges)
					{
						...
						reporter.incrCounter("Pga", "linksremoved", 1);
					}
					
					int threadsremoved = node.cleanThreads();
					reporter.incrCounter("Pga", "threadsremoved", threadsremoved);
				}
	...
	{ 
		...
		conf.setJobName("RemoveLowCoverage " + inputPath + " " + PgaConfig.K);
