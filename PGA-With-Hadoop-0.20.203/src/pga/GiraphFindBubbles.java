...
public class GiraphFindBubbles
{	
	private static final Logger sLogger = Logger.getLogger(GiraphFindBubbles.class);
	...
	public static class GiraphFindBubblesMapper 
	{
		private static int K = 0;
		public static long MAXBUBBLELEN = 0;
		BubbleComputation bubcom = null;
		private long potentialbubbles;
		MergeComputation merge = null;
		
		GiraphFindBubblesMapper(BubbleComputation bubblecomputation)
		{
			this.bubcom= bubblecomputation;
		}
		GiraphFindBubblesMapper(MergeComputation mergecomputation)
		{
		...
		public void setMaxBubLen(long MAXBUBBLELEN)
		{
			this.MAXBUBBLELEN= MAXBUBBLELEN;
		}
		...
		throws IOException 
		{
			...
			if (node.len() < MAXBUBBLELEN)
			{
				...
				{
					//this.bubcom.aggregate(POTENTIALBUBBLES, new LongWritable(1));
					this.merge.aggregate(POTENTIALBUBBLES, new LongWritable(1));
					potentialbubbles++;	
					...
					float cov = node.cov();
					/*
					bubcom.sendMessage(new Text(major), 
							new Text(Node.BUBBLELINKMSG + "\t" + 
									majord + "\t" + node.getNodeId() + "\t" + 
									minord + "\t" + minor + "\t" +
									str + "\t" + cov));
									*/
					this.merge.sendMessage(new Text(major), 
							new Text(Node.BUBBLELINKMSG + "\t" + 
									majord + "\t" + node.getNodeId() + "\t" + 
									minord + "\t" + minor + "\t" +
									str + "\t" + cov));
	...
	public static class GiraphFindBubblesReducer
	{
		private static int K = 0;
        ...
        GiraphFindBubblesReducer(BubbleComputation bubblecomputation)
		{
			this.bubcom= bubblecomputation;
		}
        GiraphFindBubblesReducer(MergeComputation mergecomputation)
        {
        	this.merge = mergecomputation;
        }
        public void setReads(int k)
		...
		public void setMaxEditRate(float BUBBLEEDITRATE)
		{
			this.BUBBLEEDITRATE= BUBBLEEDITRATE;
		}
				...
				throws IOException 
		{
			...
			while(iter.hasNext())
			{
				...
				String [] vals = msg.split("\t");
				 ...
				 if (vals[0].equals(Node.BUBBLELINKMSG))
				{
