...
{	
    ...
    implements Mapper<LongWritable, Text, Text, Text> 
	{
		...
		throws IOException 
		{
			...
			if (node.len() < MAXBUBBLELEN)
			{
				...
				{
					reporter.incrCounter("Pga", "potentialbubbles", 1);
				...
				throws IOException 
		{
			...
			while(iter.hasNext())
			{
				...
				else if (vals[0].equals(Node.BUBBLELINKMSG))
				{
					...
					reporter.incrCounter("Pga", "linkschecked", 1);
			...
			if (bubblelinks.size() > 0)
			{
				...
				for (String minor : bubblelinks.keySet())
				{
					...
					reporter.incrCounter("Pga", "minorchecked", 1);
					
					if (choices > 1)
					{
						reporter.incrCounter("Pga", "edgeschecked", choices);
						...
						for (int i = 0; i < choices; i++)
						{
							...
							for (int j = i+1; j < choices; j++)
							{
								...
								reporter.incrCounter("Pga", "bubbleschecked", 1);
								...
								if (distance <= threshold)
								{
									...
									popped++;
									reporter.incrCounter("Pga", "poppedbubbles", 1);
	...
	{ 
		...
		conf.setJobName("FindBubbles " + inputPath + " " + PgaConfig.K);
